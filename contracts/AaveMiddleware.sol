...
contract AaveMiddleware{
    ...
    function getLendingPoolAddress() public view returns(address){
        ...
        return ILendingPoolAddressesProvider(address(0xB53C1a33016B2DC2fF3653530bfF1848a515c8c5)).getLendingPool(); //Mainnet
    }

    function depositToken(uint _amount,address _tokenAddr) external {
    ...
    function withdrawToken(uint _amount,address _tokenAddr) external {
    ...
    function borrowToken(uint _amount,address _tokenAddr) external {
        address TokenAddress=_tokenAddr; //Token Address on Mainnet
        IERC20 erc20Token=IERC20(TokenAddress);    
        address _LendingPoolAddress=getLendingPoolAddress();
        ILendingPool LendingPool=ILendingPool(_LendingPoolAddress);
    ...
    function repayToken(uint _amount,address _tokenAddr) external {
        address TokenAddress=_tokenAddr; //Token Address on Mainnet
        IERC20 erc20Token=IERC20(TokenAddress);    
        address _LendingPoolAddress=getLendingPoolAddress();
        ILendingPool LendingPool=ILendingPool(_LendingPoolAddress);
    ...
    event ValueReceived(address user, uint amount);     //Event for Receive
        
    receive() external payable {
        emit ValueReceived(msg.sender, msg.value);
    }

    event Log(string func,address sender,uint value,bytes data);    //Event for Fallback
